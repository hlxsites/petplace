...
const {
  ...
  duplicatedSearchModalTitle,
  duplicatedSearchModalDescription,
  duplicatedSearchModalBackButton,
  duplicatedSearchModalManageButton,
} = placeholders;
...
function openDuplicatedSearchModal() {
  const modal = document.querySelector('#duplicated-search-modal');
  modal.classList.add('show');
  const overlay = document.querySelector('.overlay');
  overlay.classList.add('show');
}

function closeDuplicatedSearchModal() {
  const modal = document.querySelector('#duplicated-search-modal');
  modal.classList.remove('show');
  const overlay = document.querySelector('.overlay');
  overlay.classList.remove('show');
...
export async function saveSearch(token) {
  ...
  const arrSavedSearchesIds = [];
  let currentSearchId = 0;
  let isDuplicatedSearch = false;
  let maxAlertReached = false;

  await fetch(`${endPoints.apiUrl}/adopt/api/UserSearch`, {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${token}`,
    },
  }).then((response1) => response1.json()).then(async (data) => {
    data.forEach((item) => {
      arrSavedSearchesIds.push(item.Id);
    });
  });
  ...
  }).then((searchId) => searchId.json()).then((data) => {
    if (typeof data === 'number') {
      currentSearchId = data;
      if (arrSavedSearchesIds.includes(parseInt(currentSearchId, 10))) {
        isDuplicatedSearch = true;
      }
    } else {
      maxAlertReached = true;
    }
  });

  if (
    maxAlertReached
    || (response && (response.status === 204 || response.status === 400 || response.status === 500))
  ) {
    ...
    if (isDuplicatedSearch) {
      // build and show search duplicated modal
      const savedSearchModal = document.getElementById('saved-search-modal');
      if (savedSearchModal) {
        savedSearchModal.innerHTML = '';
        savedSearchModal.classList.remove('show');
      }
      let modal = document.getElementById('duplicated-search-modal');
      const resultsContainer = document.getElementById('results-container');
      if (!modal) {
        modal = document.createElement('div');
        modal.id = 'duplicated-search-modal';
        modal.className = 'duplicate-search-modal';
      } else {
        modal.innerHTML = '';
      }

      const modalTitle = document.createElement('h3');
      modalTitle.innerText = duplicatedSearchModalTitle;
      modalTitle.className = 'modal-title';

      const modalSubtitle = document.createElement('div');
      modalSubtitle.innerText = duplicatedSearchModalDescription;
      modalSubtitle.className = 'modal-subtitle';
      ...
      const manageButton = document.createElement('button');
      manageButton.className = 'manage-button';
      manageButton.innerText = duplicatedSearchModalManageButton;
      manageButton.addEventListener('click', () => {
        window.location.href = '/pet-adoption/account#searchalerts';
      });
      ...
      const continueButton = document.createElement('button');
      continueButton.className = 'continue-button';
      continueButton.innerText = duplicatedSearchModalBackButton;
      continueButton.addEventListener('click', closeDuplicatedSearchModal);
