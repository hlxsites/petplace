...
async function createChecklistSection() {
    ...
    checklistContainer.append(createCta('/pet-adoption/checklist', 'View Full Checklist', 'pet-details-button button primary right-arrow', true));
...
async function createSimilarPetsSection(sectionTitle, petArr) {
    ...
    if (petArr && petArr.length > 0) {
        ...
        petArr.forEach((pet) => {
            gallery.append(buildPetCard(pet, getMetadata('animal-card-image-fall-back')));
        });
...
export default async function decorate(block) {
    ...
    if (animalId && clientId) {
        ...
        const similarPetsArr = await fetchSimilarPets(petData.zip, petData.animalType, petListApi);
