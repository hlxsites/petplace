...
function createBanner(bannerSection) {
  ...
  div.append(...content);
  const acceptAllButton = `<button class="consent-button accept primary">${getPlaceholder('consentAcceptAll')}</button>`;
  const rejectAllButton = `<button class="consent-button decline secondary">${getPlaceholder('consentRejectAll')}</button>`;
  const moreInfoButton = `<button class="consent-button more-info secondary">${getPlaceholder('consentCustomize')}</button>`;
  const buttonsHTML = `${buttonsArray.includes('accept_all') ? acceptAllButton : ''}${buttonsArray.includes('deny_all') ? rejectAllButton : ''}${buttonsArray.includes('more_info') ? moreInfoButton : ''}`;
  if (buttonsHTML) {
