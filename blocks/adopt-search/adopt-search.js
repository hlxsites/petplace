...
async function callAnimalList(method, path, payload) {
  const baseUrl = 'https://api-stg-petplace.azure-api.net/breed/';
...
export default async function decorate(block) {
  ...
  function updateBreedOptions(options) {
  ...
  const radioBtns = radioContainer.querySelectorAll('input[name="filterAnimalType"]');

  radioBtns.forEach((radio) => {
    radio.addEventListener('change', () => {
      const selectedPetType = radio.value;

        callAnimalList('GET', selectedPetType).then((data) => {
            updateBreedOptions(data);
        });
    });
  });
  ...
  form.addEventListener('submit', (ev) => {
    ...
    const selectedAnimalType = encodeURIComponent(radioContainer.querySelector('input[name="filterAnimalType"]:checked').value.toLowerCase());
