...
function getTemplate(url) {
  ...
  //(TODO:xfeng) remove drafts/xfeng after merge to main
  if (pathname.startsWith("drafts/xfeng/traveling-with-a-pet")) {
    return "traveling-page";
  }
  return "default-page";
}
...
function fixLists(block) {
  block.querySelectorAll("li").forEach((li) => {
    if (!["OL", "UL"].includes(li.parentElement.nodeName)) {
      ...
      [...li.querySelectorAll("p,h1,h2,h3,h4,h5,h6")]
        .reverse()
        .forEach((el) => li.after(el));
      li.remove();
    }
  });
}
...
function transformArticlePage(document) {
  ...
  const toc = [...document.querySelectorAll("h2")].find(
    (h2) => toClassName(h2.textContent) === "table-of-contents"
  );

  const meta = getDefaultMetadata(document);
  meta.Author = document.querySelector(".author-name").textContent;
  meta["Publication Date"] = new Date(
  ...
  if (petInsuranceQuote) {
    const cells = [["Pet Insurance Quote"], [""]];
...
function transformBreedPage(document) {
  ...
  meta.Author = document
    ...
    .innerHTML.replace(/<svg[\s\S]*\/svg>/, "");
  meta.Type = [...document.querySelectorAll(".general-attributes")].find(
    (attr) => attr.firstElementChild.textContent === "Type"
  ).children[1].textContent;

  const generalAttributes = document.querySelector(
    ".general-attributes-wrapper"
  );
  ...
  const attributesSection = document.querySelector(
    ".is-hidden-touch .attributes-wrapper"
  );
  ...
  const historyWrapper = document.querySelector(
    ".is-hidden-touch .history-wrapper"
  );
  if (historyWrapper) {
    const cells = [["Blade"], [...historyWrapper.querySelectorAll(".column")]];
    const table = WebImporter.DOMUtils.createTable(cells, document);
    historyWrapper.replaceWith(table);
    table.after(document.createElement("hr"));
  }

  const careSection = document.querySelector(
    ".is-hidden-desktop .care-section"
  );
  if (careSection) {
    let cells = [
      ["Care Tabs"],
      ...[
        ...careSection.querySelectorAll(
          ".slick-slide:not(.slick-cloned) .care-slider"
        ),
      ].map((slider) => {
        const heading = document.createElement("h3");
        heading.textContent = slider.children[0].textContent;
        slider.children[1].prepend(heading);
        return [slider.children[0], slider.children[1]];
      }),
    ];
    let table = WebImporter.DOMUtils.createTable(cells, document);
    document
      .querySelector(".is-hidden-touch .care-section")
      .querySelector(".care-columns")
      .replaceWith(table);

    cells = [
      ["Section Metadata"],
      ...Object.entries({ Style: "Centered" }).map(([key, value]) => [
        key,
        value,
      ]),
    ];
  ...
  const breedStandardsSection = document.querySelector(
  ...
  if (breedStandardsSection) {
    let cells = [
      ["Accordion"],
      ...[
        ...breedStandardsSection.querySelectorAll(
          ".breed-standard-accordion > div"
        ),
      ].map((accordion) => [
    ...
    breedStandardsSection
      .querySelector(".breed-standard-accordion")
      .replaceWith(table);
    ...
    let cells = [
      ...
      ...[
        ...factsSection.querySelectorAll(
          ".slick-slide:not(.slick-cloned) .fact-slide-content p"
        ),
      ].map((slide) => [slide]),
    ];
    let table = WebImporter.DOMUtils.createTable(cells, document);
  ...
  const breedsToExploreSection = document.querySelector(
    ".breeds-to-explore-section"
  );
  if (breedsToExploreSection) {
    let cells = [
      ["Slide Cards (media)"],
      ...[
        ...breedsToExploreSection.querySelectorAll(
          ".breeds_to_explore-content"
        ),
      ].map((slide) => {
        const title = slide.querySelector(
          ".breeds_to_explore-title"
        ).textContent;
        ...
        anchor.append(title);
        return [slide.querySelector("picture"), anchor];
      }),
    ];
    let table = WebImporter.DOMUtils.createTable(cells, document);
  ...
  const cells = [
    ["Section Metadata"],
    ...Object.entries({ Style: "Centered, Well" }).map(([key, value]) => [
      key,
      value,
    ]),
  ];
...
function cleanupUrls(main) {
  main.querySelectorAll("[href],[src]").forEach((el) => {
    if (
      el.href &&
      el.href.startsWith("https://petplace.uat.petpartners.com/")
    ) {
      ...
      el.src &&
      el.src.startsWith("https://petplace.uat.petpartners.com/")
    ) {
...
function transformLongLists(main) {
  main.querySelectorAll(".four-column-list").forEach((list) => {
    list.outerHTML = `
        <table>
...
function dataURLtoFile(dataurl) {
  ...
  return new Blob([new Uint8Array(bytesArray)], {
