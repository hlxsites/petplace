 ...
 * @returns {Promise<ConfigAll>} the helix project config or null
 */
async function handleResponse(log, response, key, type) {
  ...
  if (response.ok) {
    ...
    log.info(`Empty or invalid /.helix/config-all.json found in ${type} ${key}`);
    return null;
  }

  if (response.status === 404) {
    log.info(`No /.helix/config-all.json found in ${type} ${key}, ${text}`);
    return null;
  }

  log[logLevelForStatusCode(response.status)](
    `Invalid response (${response.status}) when fetching config-all from ${type} ${key}`,
  );
  const err = new Error(`Unable to fetch config-all: ${response.status} ${text}`);
 ...
 * @param {PathInfo} opts lookup options
 * @returns {Promise<ConfigAll>} the project configuration
 */
export async function fetchConfigAll(ctx, opts) {
  const { log, attributes } = ctx;
  const partition = opts.route === 'preview' || opts.route === 'live' ? opts.route : 'preview';
  ...
  if (attributes.configAll === undefined) {
    attributes.configAll = await getConfig();
  }
  return attributes.configAll;
}
