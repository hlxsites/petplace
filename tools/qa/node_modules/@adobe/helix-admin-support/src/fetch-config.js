...
async function handleResponse(log, response, key, type) {
  ...
  if (response.ok) {
    ...
    try {
      ...
      props = json.data || json.default?.data;
      // eslint-disable-next-line no-empty
    } catch (ignore) { }
    if (!props) {
      log.info(`Empty or invalid .helix/config.json found in ${type} ${key}`);
      return null;
    }
    return flatJson2object(props);
  }

  if (response.status === 404) {
    log.info(`No .helix/config.json found in ${type} ${key}, ${text}`);
    return null;
  }

  log[logLevelForStatusCode(response.status)](
    `Invalid response (${response.status}) when fetching project configuration from ${type} ${key}`,
  );
  const err = new Error(`Unable to fetch project configuration: ${response.status} ${text}`);
 ...
 */
export async function fetchProjectConfig(ctx, opts) {
  ...
  const getConfig = async () => {
    ...
    const key = `${contentBusId}/preview/.helix/config.json`;
    const response = await fetchS3(ctx, 'helix-content-bus', key);
    return handleResponse(log, response, key, 'content-bus');
  };

  if (attributes.projectConfig === undefined) {
    attributes.projectConfig = await getConfig();
  }
  return attributes.projectConfig;
}
